...
test <- tableby(ASSIGN ~ AGEGROUP + RACE_ETH + SIBIRTH + WTHRatio, cat.test = "fe", numeric.test = "kwt", simulate.p.value = TRUE, numeric.stats = "medianq1q3" , data= sub.data)

table1 <- tableby(ASSIGN ~ 
                    chisq(AGEGROUP, "countrowpct") +
                    fe(AGEGROUP, simulate.p.value=TRUE ) +
                    chisq(RACE_ETH, "countrowpct") +
                    #fisher.test(RACE_ETH, )
                    ...
                    chisq(DIABF, "countrowpct"),
                  #fe(DIABF, "countrowpct", )
