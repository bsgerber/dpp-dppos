...
m1tidyfull <- tidy(m1full, conf.int = TRUE, conf.level = 0.95)
...
# Calculate predicted probability of diabetes at 3 years based on cph model
# survival ROC curve using example from https://datascienceplus.com/time-dependent-roc-for-survival-prediction-models-in-r/
#sub.data$survprob <- exp(-predict(m1full, newdata=newdf, type="expected"))

sub.data$survprob <- predict(m1full, type = "lp")

fullROC <- survivalROC(Stime = sub.data$DIABT, 
            status = ifelse(sub.data$DIABF=='Yes',1,0),
            marker = sub.data$survprob, 
            method = "KM", 
            predict.time = 3)

FULLROC.df <- data.frame(FP = fullROC$FP, TP = fullROC$TP)
  ggplot (mapping = aes (x = FP, y = TP), data = FULLROC.df) +
  geom_point(size = .25) +
  geom_line (color = "blue") +
      geom_abline(intercept  = 0, slope = 1, color = "gray", size = 0.6) +
    labs (title = "Survival ROC - Full Model", y = "True Positive Rate", x = "False Positive Rate")
...
sub.data$survprobbare <- predict(m1Bare, type = "lp")

bareROC <- survivalROC(Stime = sub.data$DIABT, 
            status = ifelse(sub.data$DIABF=='Yes',1,0),
            marker = sub.data$survprobbare, 
            method = "KM", 
            predict.time = 3)

bareROC.df <- data.frame(FP = bareROC$FP, TP = bareROC$TP)
  ggplot (mapping = aes (x = FP, y = TP), data = bareROC.df) +
  geom_point(size = .25) +
  geom_line (color = "red") +
    geom_abline(intercept  = 0, slope = 1, color = "gray", size = 0.6) +
  labs (title = "Survival ROC - Parsimonious Model", y = "True Positive Rate", x = "False Positive Rate")
  bareROC
  
  # B<- ggplot +
   #   geom_line(mapping = aes (x = FP, y = TP), data = bareROC.df, color = "red") +
  #    geom_line(mapping = aes (x = FP, y = TP), data = FULLROC.df, color = "blue") +
 # geom_point(size = .25) 
